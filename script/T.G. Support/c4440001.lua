...
-- Replacement effect for "T.G." Synchro Monsters
...
-- Set "T.G." Spell/Trap condition
function c4440001.setcond(e,tp,eg,ep,ev,re,r,rp)
    return e:GetHandler():IsFaceup()
end

-- Set "T.G." Spell/Trap target
function c4440001.settg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(c4440001.filter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil) end
end

function c4440001.filter(c)
    return c:IsSetCard(0x27) and (c:IsType(TYPE_SPELL) or c:IsType(TYPE_TRAP)) and c:IsSSetable()
end

-- Set "T.G." Spell/Trap operation
function c4440001.setop(e,tp,eg,ep,ev,re,r,rp)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SET)
    local g=Duel.SelectMatchingCard(tp,c4440001.filter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil)
    if g:GetCount()>0 then
        Duel.SSet(tp,g)
