...
function s.discardtg(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    Duel.SetOperationInfo(0,CATEGORY_DISCARD,nil,dc,tp,LOCATION_HAND)
end

function s.discardop(e,tp,eg,ep,ev,re,r,rp)
    local dc=Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)
    if dc>0 then
        Duel.DiscardHand(tp,nil,1,dc,REASON_EFFECT+REASON_DISCARD)
    else
        -- Optionally, you can add a hint message here if no cards are available to discard
        Duel.Hint(HINT_MESSAGE,tp,HINTMSG_DISCARD)
    end
end
